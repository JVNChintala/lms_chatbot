...
class SimpleCanvasAgent:
    def __init__(self, canvas_url: str, canvas_token: str, user_id: int = None):
        print(f"SimpleCanvasAgent initialized for user_id: {user_id}")
    ...
    def process_message(self, user_message: str, conversation_history: list, user_role: str = None, user_info: dict = None) -> dict:
        """Process user message with simple tool detection"""
        if user_role:
    ...
    def _generate_simple_response(self, user_message):
        ...
        if any(word in msg for word in ['hello', 'hi', 'hey', 'good morning', 'good afternoon']):
            role_text = f" I can help you with Canvas LMS as a {self.user_role}." if self.user_role else ""
            return {"content": f"Hello!{role_text} Try asking me to 'list courses', 'create a course', or 'show modules'.", "tool_used": False}
        
        # Help requests
        if any(word in msg for word in ['help', 'what can you do', 'commands', 'options']):
            if self.user_role == "admin":
                commands = "• list courses\\n• create course [name]\\n• list modules\\n• create module [name]\\n• create assignment [name]\\n• list users"
            elif self.user_role == "teacher":
                commands = "• list courses\\n• create course [name]\\n• list modules\\n• create module [name]\\n• create assignment [name]"
            else:
                commands = "• list courses\\n• list modules"
            
            return {"content": f"I can help you with these Canvas LMS commands:\\n{commands}", "tool_used": False}
        
        # Course-related queries
        if 'course' in msg:
            return {"content": "I can help with courses! Try: 'list courses' to see all courses, or 'create course [name]' to make a new one.", "tool_used": False}
        
        # Module-related queries
        if 'module' in msg:
            return {"content": "I can help with modules! Try: 'list modules' to see modules, or 'create module [name]' to add one.", "tool_used": False}
        
        # Assignment-related queries
        if 'assignment' in msg:
            return {"content": "I can help with assignments! Try: 'create assignment [name]' to create a new assignment.", "tool_used": False}
        
        # Default response
        return {"content": "I'm a Canvas LMS assistant. I can help you list courses, create courses, manage modules, and create assignments. What would you like to do?", "tool_used": False}
